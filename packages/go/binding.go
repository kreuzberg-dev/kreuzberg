...
jsonBytesRecv, err := json.Marshal(r) if err != nil {
return fmt.Errorf("failed to marshal receiver: %w", err)
} tmpStrRecv := C.CString(string(jsonBytesRecv)) cRecv := C.xberg_extraction_config_from_json(tmpStrRecv)
C.free(unsafe.Pointer(tmpStrRecv)) if cRecv == nil {
...
jsonBytesRecv, err := json.Marshal(r) if err != nil {
return fmt.Errorf("failed to marshal receiver: %w", err)
} tmpStrRecv := C.CString(string(jsonBytesRecv)) cRecv := C.xberg_llm_config_from_json(tmpStrRecv)
C.free(unsafe.Pointer(tmpStrRecv)) if cRecv == nil {
return fmt.Errorf("failed to create receiver: %s", C.GoString(C.xberg_last_error_context()))
}
...
jsonBytesRecv, err := json.Marshal(r) if err != nil {
return fmt.Errorf("failed to marshal receiver: %w", err)
} tmpStrRecv := C.CString(string(jsonBytesRecv)) cRecv := C.xberg_redaction_config_from_json(tmpStrRecv)
C.free(unsafe.Pointer(tmpStrRecv)) if cRecv == nil {
return fmt.Errorf("failed to create receiver: %s", C.GoString(C.xberg_last_error_context()))
}
...
jsonBytesRecv, err := json.Marshal(r) if err != nil {
return "", fmt.Errorf("failed to marshal receiver: %w", err)
} tmpStrRecv := C.CString(string(jsonBytesRecv)) cRecv := C.xberg_server_config_from_json(tmpStrRecv)
C.free(unsafe.Pointer(tmpStrRecv)) if cRecv == nil {
return "", fmt.Errorf("failed to create receiver: %s", C.GoString(C.xberg_last_error_context()))
}
...
} tmpStrRecv := C.CString(string(jsonBytesRecv)) cRecv := C.xberg_footnote_config_from_json(tmpStrRecv)
C.free(unsafe.Pointer(tmpStrRecv)) if cRecv == nil {
...
jsonBytesRecv, err := json.Marshal(r) if err != nil {
return fmt.Errorf("failed to marshal receiver: %w", err)
} tmpStrRecv := C.CString(string(jsonBytesRecv)) cRecv := C.xberg_document_structure_from_json(tmpStrRecv)
C.free(unsafe.Pointer(tmpStrRecv)) if cRecv == nil {
return fmt.Errorf("failed to create receiver: %s", C.GoString(C.xberg_last_error_context()))
}
...
jsonBytesRecv, err := json.Marshal(r) if err != nil {
return false, fmt.Errorf("failed to marshal receiver: %w", err)
} tmpStrRecv := C.CString(string(jsonBytesRecv)) cRecv := C.xberg_metadata_from_json(tmpStrRecv)
C.free(unsafe.Pointer(tmpStrRecv)) if cRecv == nil {
return false, fmt.Errorf("failed to create receiver: %s", C.GoString(C.xberg_last_error_context()))
}
...
jsonBytesRecv, err := json.Marshal(r) if err != nil {
return false, fmt.Errorf("failed to marshal receiver: %w", err)
} tmpStrRecv := C.CString(string(jsonBytesRecv)) cRecv := C.xberg_multi_vector_embedding_from_json(tmpStrRecv)
C.free(unsafe.Pointer(tmpStrRecv)) if cRecv == nil {
return false, fmt.Errorf("failed to create receiver: %s", C.GoString(C.xberg_last_error_context()))
}
...
jsonBytesRecv, err := json.Marshal(r) if err != nil {
return fmt.Errorf("failed to marshal receiver: %w", err)
} tmpStrRecv := C.CString(string(jsonBytesRecv)) cRecv := C.xberg_heuristics_config_from_json(tmpStrRecv)
C.free(unsafe.Pointer(tmpStrRecv)) if cRecv == nil {
return fmt.Errorf("failed to create receiver: %s", C.GoString(C.xberg_last_error_context()))
}
...
jsonBytesRecv, err := json.Marshal(r) if err != nil {
return 0, fmt.Errorf("failed to marshal receiver: %w", err)
} tmpStrRecv := C.CString(string(jsonBytesRecv)) cRecv := C.xberg_page_range_from_json(tmpStrRecv)
C.free(unsafe.Pointer(tmpStrRecv)) if cRecv == nil {
return 0, fmt.Errorf("failed to create receiver: %s", C.GoString(C.xberg_last_error_context()))
}
...
jsonBytesRecv, err := json.Marshal(r) if err != nil {
return false, fmt.Errorf("failed to marshal receiver: %w", err)
} tmpStrRecv := C.CString(string(jsonBytesRecv)) cRecv := C.xberg_doctor_report_from_json(tmpStrRecv)
C.free(unsafe.Pointer(tmpStrRecv)) if cRecv == nil {
return false, fmt.Errorf("failed to create receiver: %s", C.GoString(C.xberg_last_error_context()))
}
...
jsonBytesRecv, err := json.Marshal(r) if err != nil {
return nil, fmt.Errorf("failed to marshal receiver: %w", err)
} tmpStrRecv := C.CString(string(jsonBytesRecv)) cRecv := C.xberg_paddle_ocr_config_from_json(tmpStrRecv)
C.free(unsafe.Pointer(tmpStrRecv)) if cRecv == nil {
return nil, fmt.Errorf("failed to create receiver: %s", C.GoString(C.xberg_last_error_context()))
}
