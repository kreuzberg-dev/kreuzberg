        ...
        match read_file_from_zip(archive, &file_path) {
            Ok(raw_xhtml) => {
                ...
                if nesting_depth_exceeds(&normalized_xhtml, depth_limit) {
                    ...
                    let text = strip_html_tags(&normalized_xhtml);
                    if !text.is_empty() {
                ...
                let chapter = extract_text_from_xhtml_reporting(&render_xhtml);
...
pub(super) fn extract_text_from_xhtml(xhtml: &str) -> String {
    extract_text_from_xhtml_reporting(xhtml).text
}
...
/// Uses the default security limits.
pub(super) fn extract_text_from_xhtml_reporting(xhtml: &str) -> ChapterText {
    let mut budget = SecurityBudget::with_defaults();
    extract_text_from_xhtml_with_budget(xhtml, &mut budget)
}
...
pub(super) fn normalize_xhtml(xml: &str) -> String {
    expand_named_entities(&strip_serialized_mathml_comments(&strip_xml_prelude(xml)))
}
...
pub(super) fn expand_named_entities(xhtml: &str) -> String {
    if !xhtml.contains('&') {
        return xhtml.to_string();
    }
    ...
    while let Some(start) = rest.find('&') {
        ...
        let replacement = (name_len > 0 && candidate.as_bytes().get(name_len + 1) == Some(&b';'))
            ...
            .filter(|reference| !matches!(*reference, "&amp;" | "&lt;" | "&gt;" | "&quot;" | "&apos;"))
            .and_then(|reference| {
