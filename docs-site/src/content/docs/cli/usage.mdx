...
`doctor` checks whether the backends in your config will actually run on this machine, before the first document. Each check reports pass, fail, or skip with a one-line reason; the command exits nonzero on any failure.
